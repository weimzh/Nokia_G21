...
function do_make_modules()
{
	...
	cd $BSP_ROOT_DIR
	BSP_MOD_PATH_LIST=`find ./modules -type f -name "Makefile" | grep -E "$BSP_KERNEL_VERSION|camera"`
	...
	if [[ -z $specified_ko ]]; then
		...
		do
			...
			do
				BSP_MOD_PATH=$BSP_ROOT_DIR/$BSP_MOD_PATH
				compile_modules
				if [ $? -ne 0 ]; then
					return_val=1
					failure_handle $BSP_MOD.ko
...
do_make_modules $@
kernelrelease=$(find $BSP_MODULES_OUT -mindepth 3 -maxdepth 3 |grep "lib/modules/" |awk -F "lib/modules/" '{print $2}')
echo "DEPMOD  ALL $kernelrelease"
/sbin/depmod -ae -F $BSP_KERNEL_OUT/System.map -b $BSP_MODULES_OUT $kernelrelease
