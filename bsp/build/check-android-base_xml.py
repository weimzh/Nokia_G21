	...
	for group in d_baseconfig:
		...
		if condition_match == len(d_baseconfig[group]['condition']):
			for config in d_baseconfig[group]['result']:
				if "DISALLOW" == config[0:8]:
					real_config = re.split("DISALLOW_|_IS_N",config).pop(1)
					if real_config not in d_productconfig or d_productconfig[real_config] == "n":
						print("ERROR configuration: {} : The status cannot be [{}] in .config, because that is disallowed in base.xml."\
							.format(real_config,d_productconfig[real_config])),
						print("Should modify {} to satisfy requirement of android-base-condtional.xml".format(real_config))
						return_val = 1
						continue
				if config in d_productconfig:
