...
// ning.wei++
extern ssize_t sprd_get_wakeup_source(char *buf);
extern sprd_clear_wakeup_source_log_buf(void);
extern ssize_t hmd_get_rtc_wakeup_addition_info(char *buf);
extern void hmd_clear_rtc_wakeup_addition_info(void);
...
void clear_wakeup_reasons(void)
{
	...
	#ifdef CONFIG_SPRD_POWER_LOG
	// ning.wei++ for battery historian show wake up reason
	sprd_clear_wakeup_source_log_buf();
	hmd_clear_rtc_wakeup_addition_info();
	#endif
				       ...
				       struct kobj_attribute *attr, char *buf)
{
	...
	#ifdef CONFIG_SPRD_POWER_LOG // ning.wei++ for battery historian show wake up reason
	else {
		buf_offset = sprd_get_wakeup_source(buf);
		if (strcmp(buf, "-1 ANA(RTC_INT)") == 0) {
			buf_offset += hmd_get_rtc_wakeup_addition_info(buf+buf_offset);
		}
	}
	#endif
