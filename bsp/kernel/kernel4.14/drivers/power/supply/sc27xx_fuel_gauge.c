...
#include <linux/hardware_info.h>//add by wt.liuzhiqing for SCT-702, hardware info: fuel-gauge info
...
//+SCT-702, fangduozhu.wt, add, battery bringup
...
//+SCT-702, fangduozhu.wt, add, battery bringup
#define BATID_PULLUP_VOL_MV     (1850)
#define BATID_PULLUP_RES_OHM    (100000)
static int sc27xx_fgu_get_batid(struct sc27xx_fgu_data *data, int *batid)
{
	int ret, res, i=0;
	unsigned int res_low_limit, res_high_limit;
	struct device_node *battery_np = NULL;

	ret = iio_read_channel_processed(data->batid_channel, &res);//res vol(mV)
	if (ret < 0) {
		dev_err(data->dev, "IIO fail to read battery id resistance<ret:%d>!!\n", ret);
		*batid = -1;
		return ret;
	}
	res = BATID_PULLUP_RES_OHM*res/(BATID_PULLUP_VOL_MV-res);//res(ohm)

	while (true) {
		battery_np = of_parse_phandle(data->battery->of_node, "monitored-battery", i);
		if (!battery_np) {
			dev_info(data->dev,"failed to get battery_np of bat%d!!!\n", i);
			break;
		}
		if (of_property_read_u32_index(battery_np, "id-resistance-ohm-range", 0, &res_low_limit)
			|| of_property_read_u32_index(battery_np, "id-resistance-ohm-range", 1, &res_high_limit))
			break;;
		dev_info(data->dev, "bat%d id resitance range is %d~%dohm\n", i, res_low_limit, res_high_limit);
		if (res>=res_low_limit && res<=res_high_limit) {
			*batid = i;
			return res;
		}
		i++;
	...
	dev_err(data->dev, "non-standard battery!!!\n");
	*batid = 2;
	return res;
}
//-SCT-702, fangduozhu.wt, add, battery bringup
...
//+SCT-702, fangduozhu.wt, add, battery bringup
	case POWER_SUPPLY_PROP_BATT_ID_SELECT:
        	if (data->batid == -1) {
			ret = sc27xx_fgu_get_batid(data, &data->batid);
			if (ret<0) {
				dev_err(data->dev, "failed to get battery id!\n");
				break;
			}
			dev_info(data->dev, "battery id resistance is %dohm, bat_id is %d\n", ret, data->batid);
		}
		val->intval = data->batid;
		break;
//-SCT-702, fangduozhu.wt, add, battery bringup
...
//SCT-702, fangduozhu.wt, add, battery bringup
	POWER_SUPPLY_PROP_BATT_ID_SELECT,
};
...
#define BMS_GAUGE_INFO		"UMP510G"	//add by wt.liuzhiqing for SCT-702, hardware info: fuel-gauge info
			      ...
			      const struct sc27xx_fgu_variant_data *pdata)
{
	...
	hardwareinfo_set_prop(HARDWARE_BMS_GAUGE, BMS_GAUGE_INFO);	//add by wt.liuzhiqing for SCT-702, hardware info: fuel-gauge info
	return 0;
...
static int sc27xx_fgu_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(data->channel)) {
		dev_err(&pdev->dev, "failed to get bat-temp IIO channel\n");
		return PTR_ERR(data->channel);
	}

	data->charge_cha = devm_iio_channel_get(&pdev->dev, "charge-vol");
	if (IS_ERR(data->charge_cha)) {
		dev_err(&pdev->dev, "failed to get charge-vol IIO channel\n");
		return PTR_ERR(data->charge_cha);
	}

//+SCT-702, fangduozhu.wt, add, battery bringup
	data->batid_channel = devm_iio_channel_get(&pdev->dev, "bat-id");
	if (IS_ERR(data->batid_channel)) {
		dev_err(&pdev->dev, "failed to get bat-id IIO channel\n");
		return PTR_ERR(data->batid_channel);
	}
	data->batid = -1;
//-SCT-702, fangduozhu.wt, add, battery bringup
