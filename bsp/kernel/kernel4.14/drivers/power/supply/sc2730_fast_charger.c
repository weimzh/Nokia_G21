...
//modify by fangduozhu.wt, max power 20w
#define SC2730_PD_DEFAULT_POWER_UW		21000000
					 ...
					 u32 input_vol)
{
	...
	tcpm_get_source_capabilities(port, &info->pd_source_cap);
	if (!info->pd_source_cap.nr_source_caps) {
//modify by fangduozhu.wt, max power 20w begin
		if (input_vol == FCHG_VOLTAGE_5V) {
			pdo[0] = PDO_FIXED(5000, 2000, 0);
			snk_uw = SC2730_PD_DEFAULT_POWER_UW;
		} else {
			pdo[0] = PDO_FIXED(9000, 2000, 0);
			snk_uw = 18000000;
		}
//modify by fangduozhu.wt, max power 20w end
		index = 0;
		goto done;
	}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = 1;
//modify by fangduozhu.wt, show max power info
		if (info->state == POWER_SUPPLY_USB_TYPE_UNKNOWN
			|| info->state == POWER_SUPPLY_USB_TYPE_C)
			val->intval = 0;
		break;
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (info->pps_enable)
			...
			val->intval = 2200000;
		else if (info->usb_phy->chg_type == DCP_TYPE)
			val->intval = 2000000;
