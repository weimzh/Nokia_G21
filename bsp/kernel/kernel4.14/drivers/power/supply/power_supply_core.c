...
#include <linux/hardware_info.h>//add by wt.liuzhiqing for SCT-702, hardware info: battery info
				  ...
				  struct power_supply_battery_info *info, int num)
{
	...
	hardwareinfo_set_prop(HARDWARE_BATTERY_ID, value);//add by wt.liuzhiqing for SCT-702, hardware info: battery info
	return 0;
}
...
#endif
//Add by wt.liuzhiqing for SCT-702,add node : real_type begin
static ssize_t show_real_type(struct device *dev,struct device_attribute *attr, char *buf)
{
	/*  real_type */
	union power_supply_propval val = {0}, val_0 = {0};
	struct power_supply	*batt_psy =NULL;
	int rc;

	batt_psy = power_supply_get_by_name("bq25890_charger");
	if(batt_psy) {
		rc = power_supply_get_property(batt_psy, POWER_SUPPLY_PROP_USB_TYPE, &val);
	} else {
		pr_err("get battery power supply Error!!\n");
	}
	batt_psy = power_supply_get_by_name("sc2730_fast_charger");
	if(batt_psy) {
		rc = power_supply_get_property(batt_psy, POWER_SUPPLY_PROP_USB_TYPE, &val_0);
	} else {
		pr_err("get battery power supply Error!!\n");
	}

	if((val_0.intval != POWER_SUPPLY_USB_TYPE_C) &&
		(val_0.intval != POWER_SUPPLY_USB_TYPE_UNKNOWN)){
		val.intval = val_0.intval;
	}
	...
	switch (val.intval) {
		...
		case POWER_SUPPLY_USB_TYPE_PD:
			return sprintf(buf, "USB_PD\n");
			break;

		case POWER_SUPPLY_USB_TYPE_PD_DRP:
			return sprintf(buf, "USB_PD_DRP\n");
			break;

		case POWER_SUPPLY_USB_TYPE_PD_PPS:
			return sprintf(buf, "USB_PD_PPS\n");
			break;

		case POWER_SUPPLY_USB_TYPE_APPLE_BRICK_ID:
			return sprintf(buf, "USB_APPLE_BRICK_ID\n");
			break;

		case POWER_SUPPLY_USB_TYPE_SFCP_1P0:
			return sprintf(buf, "USB_SFCP_1P0\n");
			break;

		case POWER_SUPPLY_USB_TYPE_SFCP_2P0:
			return sprintf(buf, "USB_SFCP_2P0\n");
			break;
...
static ssize_t store_real_type(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	return -1;
}
static DEVICE_ATTR(real_type, 0664, show_real_type, store_real_type);
//Add by wt.liuzhiqing for SCT-702,add node : real_type end
//Add by wt.liuzhiqing for SCT-702,add node : typec_cc_orientation begin
extern int cc1_cc2_check;
static ssize_t show_typec_cc_orientation(struct device *dev,struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", cc1_cc2_check);
}
static ssize_t store_typec_cc_orientation(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	return -1;
}
static DEVICE_ATTR(typec_cc_orientation, 0664, show_typec_cc_orientation, store_typec_cc_orientation);
//Add by wt.liuzhiqing for SCT-702,add node : typec_cc_orientation end
				   ...
				   bool ws)
{
	...
	struct power_supply *psy;
	int i, rc;
	int ret_device_file = 0;	//Add by wt.liuzhiqing for SCT-702,add node : real_type
	if (!parent)
...
//Add by wt.liuzhiqing for SCT-702,add node : real_type begin
	if(strcmp(psy->desc->name, "battery") == 0) {
		pr_err("battery powe supply creat real_type attr files!!\n");
		ret_device_file = device_create_file(dev, &dev_attr_real_type);
		ret_device_file = device_create_file(dev, &dev_attr_typec_cc_orientation);//Add by wt.liuzhiqing for SCT-702,add node : typec_cc_orientation
	}
//Add by wt.liuzhiqing for SCT-702,add node : real_type end
	return psy;
