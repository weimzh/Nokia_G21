...
int g_boostmode_flag = 0;
static ssize_t boostmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_boostmode_flag);
}
static ssize_t boostmode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int rc;
	unsigned long boostmode_flag;

	rc = kstrtoul(buf, 0, &boostmode_flag);
	if (rc)
		return rc;
	g_boostmode_flag = boostmode_flag;

	return rc ? rc : count;
}
static DEVICE_ATTR_RW(boostmode);
...
static struct attribute *bl_device_attrs[] = {
	...
	&dev_attr_type.attr,
	&dev_attr_boostmode.attr,	//Modify by pengzhenhua1@wingtech.com for SCT-887,wt lcd backlight boost mode on(2021.11.23)
	NULL,
};
