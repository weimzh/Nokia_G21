...
static irqreturn_t nfc_dev_irq_handler(int irq, void *dev_id)
{
    ...
    unsigned long flags;
    //Modified by wt.liyou1 for SCT-702,nfc analog card in suspend begin
    if (device_may_wakeup(&nfc_dev->client->dev))
        pm_wakeup_event(&nfc_dev->client->dev, WAKEUP_SRC_TIMEOUT);
    //Modified by wt.liyou1 for SCT-702,nfc analog card in suspend end
        ...
        size_t count, loff_t *offset)
{
    ...
    int ret;
//Modified by wt.liyou1 for SCT-702,disable nfc info begin
#if 0
    int i = 0;
#endif
//Modified by wt.liyou1 for SCT-702,disable nfc info end
    int irq_gpio_val = 0;
    if (!nfc_dev) {
        return -ENODEV;
    }
...
//Modified by wt.liyou1 for SCT-702,disable nfc info end
    ...
    pr_info("nfc_dev_write start i2c_master_send\n");
...
static int nfc_dev_open(struct inode *inode, struct file *filp)
{
    ...
    //Modified by wt.liyou1 for SCT-702,nfc analog card in suspend 
...
{
    //Modified by wt.liyou1 for SCT-702,nfc supports multiple hardware devices with one software begin
	...
	strstr(saved_command_line,"board_id=T19655WA1")){
        ...
        pr_info("The device has no NFC module\n");
        return -1;
    }
    //Modified by wt.liyou1 for SCT-702,nfc supports multiple hardware devices with one software end
}
