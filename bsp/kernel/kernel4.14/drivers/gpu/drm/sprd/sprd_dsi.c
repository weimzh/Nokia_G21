...
static void sprd_dsi_encoder_enable(struct drm_encoder *encoder)
{
	...
	struct sprd_panel *panel = NULL; //Modify by pengzhenhua1@wingtech.com for SCT-702,wt dynamic frame rate adjustment bringup on(2021.9.7)
...
static void sprd_dsi_encoder_disable(struct drm_encoder *encoder)
{
	...
	sprd_dpu_stop(dpu);
	if (dsi->ctx.dpi_clk_div) {	//Modify by pengzhenhua1@wingtech.com for SCT-702,wt dynamic frame rate adjustment bringup on(2021.10.19)
		if (!strcmp(dpu->ctx.version, "dpu-r6p0")) {
...
//Modify by pengzhenhua1@wingtech.com for SCT-702,wt dynamic frame rate adjustment bringup on(2021.9.7) begin
	...
	if (!of_property_read_u32(lcd_node, "sprd,dpi-clk-div", &val))	//Modify by pengzhenhua1@wingtech.com for SCT-702,wt dynamic frame rate adjustment bringup on(2021.10.19)
		ctx->dpi_clk_div = val;
//Modify by pengzhenhua1@wingtech.com for SCT-702,wt dynamic frame rate adjustment bringup on(2021.9.7) end
	if (dsi->dsi_slave) {
...
static int sprd_dsi_context_init(struct sprd_dsi *dsi, struct device_node *np)
{
	...
	if (!of_property_read_u32(np, "sprd,max-read-time", &tmp))
		...
		ctx->max_rd_time = 0x8000;	//Modify by pengzhenhua1@wingtech.com for SCT-702,wt dynamic frame rate adjustment bringup on(2021.9.7)
