...
char wakeup_soruce_log_buf[128] = {0};
...
static int ap_wakeup_source_print(struct device *dev, struct debug_data *dat)
{
	...
	// ning.wei++ for battery historian show wake up reason
	len = sprintf(wakeup_soruce_log_buf, "-1 %s", wakeup_name);

	if (wakeup_device) {
		pr_info("   #--%s\n", wakeup_device);
		sprintf(wakeup_soruce_log_buf+len, " %s", wakeup_device);
	}
	pr_info(" ###--END\n");
...
// Add-begin by ning.wei for battery historian show wake up reason
ssize_t sprd_get_wakeup_source(char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s", wakeup_soruce_log_buf);
}

void sprd_clear_wakeup_source_log_buf()
{
	memset(wakeup_soruce_log_buf, 0, sizeof(wakeup_soruce_log_buf));
}
// Add-end by ning.wei for battery historian show wake up reason
