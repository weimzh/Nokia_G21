...
bool jeita_camera_in_use = false;
EXPORT_SYMBOL(jeita_camera_in_use);
...
//add by fangduozhu.wt, decrease charge vol to 5V when camera opened(2021.11.10) begin
	if (!strcmp(rdev_get_name(rdev), "vddcamio"))
		jeita_camera_in_use = true;
//add by fangduozhu.wt, decrease charge vol to 5V when camera opened(2021.11.10) end
	rdev->use_count++;
...
//add by fangduozhu.wt, decrease charge vol to 5V when camera opened(2021.11.10) begin
	if (!strcmp(rdev_get_name(rdev), "vddcamio") && rdev->use_count==0)
		jeita_camera_in_use = false;
//add by fangduozhu.wt, decrease charge vol to 5V when camera opened(2021.11.10) end
	return ret;
}
