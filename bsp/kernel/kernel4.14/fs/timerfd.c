...
// ning.wei++ for battery historian log rtc wakeup process
static char rtc_addition_info[128] = {0};
...
// Add-begin by ning.wei for battery historian log rtc wakeup process
ssize_t hmd_get_rtc_wakeup_addition_info(char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s", rtc_addition_info);
}

void hmd_clear_rtc_wakeup_addition_info()
{
	memset(rtc_addition_info, 0, sizeof(rtc_addition_info));
}
// Add-end by ning.wei for battery historian log rtc wakeup process
...
static unsigned int timerfd_poll(struct file *file, poll_table *wait)
{
	...
	if (ctx->expired && isalarm(ctx)) {
		/*pr_emerg("[oem]%s: comm:%s pid:%d exp:%llu\n", __func__,
					current->comm, current->pid,
					ktime_to_ms(ctx->t.alarm.node.expires));*/
		sprintf(rtc_addition_info, ",%s", current->comm);
	}
	// Add-end by ning.wei for battery historian log rtc wakeup process
